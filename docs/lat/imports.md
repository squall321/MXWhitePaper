...
| xlsx | [[src/app/services/xlsx_import.py#xlsx_to_document]] | 시트=섹션, 표→TableBlock (크기 무관 — SpreadsheetBlock 안 만듦), embedded 차트→ChartBlock, label/value·name/start/end 표는 kpi/gantt 로 autodetect | `data_only=True` (수식 대신 캐시값) — 캐시 없으면 빈 셀. 일반 숫자 표는 차트 자동변환 안 함 (표 유지). ★ 예전엔 200행 초과를 SpreadsheetBlock(rows le=200)로 만들어 201행부터 저장 422 였음 — TableBlock(무제한)으로 수정 (adversarial-verify) |
